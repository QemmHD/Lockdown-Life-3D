...
function bed() { const g = new THREE.Group(); g.add(box(1.1, 0.3, 2.0, M.darkMetal, 0, 0.3, 0)); g.add(box(1.0, 0.18, 1.9, M.mattress, 0, 0.52, 0)); g.add(box(0.9, 0.16, 0.5, M.pillow, 0, 0.62, -0.6)); return g; }
...
function counter() { const g = new THREE.Group(); g.add(box(4, 1, 0.9, M.metal, 0, 0.5, 0)); g.add(box(4, 0.1, 1.0, M.steel, 0, 1.0, 0)); return g; }
    ...
    switch (r.type) {
      case 'cellblock': {
        ...
        for (let i = 0; i < cols; i++) {
          const x = minX + 2 + i * 3;
          place(bed(), x, minZ + 1.4);
          place(toilet(), x + 1, minZ + 3.2, Math.PI);
          place(locker(), x - 0.6, minZ + 3.2);
        }
        break;
      }
      case 'cafeteria': {
        place(counter(), cx, minZ + 1.2);
        for (let i = 0; i < 3; i++) place(table(), minX + 3 + i * 4, cz + 1.5);
        place(trash(), maxX - 1, maxZ - 1);
        break;
      }
      case 'yard': {
        place(bench(), minX + 3, minZ + 1.5);
        place(bench(), minX + 3, maxZ - 1.5);
        place(weights(), cx + 2, cz);
        place(pullup(), maxX - 3, minZ + 2);
        place(dirtPatch(), cx - 2, cz + 1); place(dirtPatch(), cx + 3, maxZ - 2);
        break;
      }
      case 'shower': {
        for (let i = 0; i < 3; i++) { place(showerHead(), minX + 1.2, minZ + 1.4 + i * 1.4, Math.PI / 2); place(drain(), minX + 3, minZ + 1.4 + i * 1.4); }
        break;
      }
      case 'guardroom': {
        place(desk(), cx, minZ + 1.4);
        place(cabinet(), maxX - 1, maxZ - 1);
        break;
      }
      case 'hallway': {
        for (let x = minX + 4; x < maxX - 2; x += 8) place(ceilingLamp(), x, cz);
        place(pipe(r.w - 6), cx, maxZ - 0.4);
        place(sign(0xd8a72c), minX + 6, minZ + 0.4);
        place(sign(0x3a7fd8), maxX - 8, maxZ - 0.4);
        break;
      }
    }
  }
