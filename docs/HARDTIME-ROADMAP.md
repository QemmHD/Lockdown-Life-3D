...
# DEEP-RESEARCH: more missed Hard Time mechanics

The single biggest miss is the two-axis social standing system (Reputation = feared/respected vs Attitude = good-citizen), where the SAME action moves the two ledgers in OPPOSITE directions for different audiences: winning fights and getting convicted raises inmate respect but makes guards and the judge more hostile and lengthens your sentence; cooperating raises Attitude but makes you look weak inside. You have "reputation/notoriety" as ONE value. Splitting it into respect+attitude and routing every NPC reaction through an OpinionSystem(observerFaction, actorRespect, actorAttitude) is the deepest, highest-leverage authenticity gain in the whole list, because it is the hidden engine behind court drama, snitching, gang fear, the "guilty men are respected / innocent verdicts are suspicious" quirk, and the menial-job reputation drain — none of which feel right with a single rep number.


## Obscure Hard Time systems we missed: requests/petitions, parole/warden meetings, visiting/phone/mail/morale activities, editors, weather/day-night, secret areas, item durability, amber energy ceiling, overdose/addiction

- **Warden-issued STAT/REP/MONEY conditions as 'release requirements' (the petition/condition system)** _(high)_ — Wardens don't just hand out one-off missions; they impose timed RELEASE CONDITIONS you must meet by end of sentence: e.g. 'raise a stat to X before day Y', 'reduce your reputation below X', or 'accumulate $X'. Meeting them shaves days; failing ADDS days with NO court case. This is the closest thing to a parole/condition system and is distinct from generic jobs. Failing a warden task lowers reputation or extends sentence.
  - add: Add a ReleaseConditions component on the player (list of {type: stat|rep|money, target, deadlineDay}); a WardenContractSystem rolls a new condition periodically and on deadline applies +days (sentence) or -days to the Sentence component.
  - src: hard-timegame.fandom.com/wiki/Warden; chaptercheats Hard Time Answers (warden tasks add/remove days, no court case)

- **Missions sourced from multiple channels including a PHONE LINE, not just NPCs** _(medium)_ — Missions/requests are issued by inmates, gang members, WARDENS, and a phone line. Time-sensitive missions appear in the main hall; completing in time shaves days, money reward ~$100 for gang missions, $5 for sweeping. The phone is an explicit mission/request source (e.g. lawyer/contact calls) we don't model.
  - add: Add a PhoneEntity + MissionOffer component with sourceType (inmate|gang|warden|phone) and an expiryTick; a MissionDispatchSystem spawns offers and routes rewards (money/sentence/rep) on completion/expiry.
  - src: hard-timegame.fandom.com/wiki/Missions; mdickiegameguide.blogspot.com (gang missions ~$100, sweep $5); chaptercheats (lawyer phone call)

- **SPIRIT / mental-health bar (separate from health) with Adrenaline Rush and Nervous Breakdown** _(high)_ — A thin yellow 'adrenaline/spirit' bar sits UNDER the health bar. Filled by positive experiences -> 'adrenaline rush': character becomes ~10% stronger in EVERY respect and triggers special/upgraded versions of moves. Exhausted spirit -> 'nervous breakdown': you LOSE CONTROL of your character (AI takes over). This is a two-state emergent system beyond a simple morale bar.
  - add: Add a Spirit component (0..max); SpiritSystem: above threshold sets AdrenalineRush status (+10% to STR/AGI/SKILL multipliers, unlock special-move flag); at zero sets Breakdown status that swaps player Controller for AIController for N ticks.
  - src: TV Tropes VideoGame/HardTime (adrenaline rush +10%, breakdown loss of control); HT3 official manual (yellow adrenaline bar under health)

- **Amber 'energy ceiling' — low health that lingers cannot be fully restored** _(medium)_ — Green health drains through the day (faster if you RUN everywhere). The longer health stays low, the harder it is to restore fully — visualized as an AMBER cap on the health bar that lowers your effective max. Eating/drinking restores small chunks. In HT3 this amber drain is OPTIONAL and OFF by default; when on, it forces you to get tired enough to sleep at night.
  - add: Add maxHealthCeiling to the Health component; a FatigueSystem lowers the ceiling while health stays below a threshold over time and raises it slowly on sleep; toggle behind a config flag.
  - src: HT3 official manual / mdickie.fandom.com/wiki/Hard_Time_III (amber limit, running drains faster); HT3 Versions patch notes (amber drain optional, off by default)

- **Weather / day-night with cold-damage hazard** _(medium)_ — Cold OUTDOOR weather damages health more rapidly, shown by a discoloured health meter and a shivering animation. So being outdoors at the wrong time/temperature is an environmental DoT — a weather layer on top of day-night, not just lighting.
  - add: Add a Weather singleton (temp by tick) + Outdoors tag on zones; WeatherHazardSystem applies health DoT + Shivering status to entities that are Outdoors while temp < threshold.
  - src: HT3 official manual / wiki (cold outdoor weather damages health, discoloured meter, shivering)

- **Drugs/alcohol/tobacco/needles affect BOTH health and spirit, with a happiness PENALTY for needles** _(medium)_ — Alcohol, tobacco and medicinal drugs are consumable and affect physical health AND mental health (spirit). Injecting random needles found on the ground heals you quickly BUT carries a penalty of LOWERING your happiness/spirit. So 'drugs' are a risk/reward heal with a morale cost rather than a pure buff — the addiction/overdose cost is modeled as spirit loss.
  - add: Give Consumable items a {healthDelta, spiritDelta} pair; NeedleSyringe = large healthDelta + negative spiritDelta; a ConsumeSystem applies both. Optional: track cumulative use for an addiction/tolerance modifier.
  - src: mdickie.fandom.com/wiki/Hard_Time_III (alcohol/tobacco/drugs affect physical+mental health; needles heal but lower happiness)

- **Crafting recipe chain: liquids + blades/tissue -> syringes/medikits (consumables you build)** _(medium)_ — Specific HT3 crafting: combine LIQUIDS with BLADES or TISSUE to create SYRINGES and MEDIKITS, which restore health/spirit faster than raw items. Crafting is also one of the listed ways to top up money/credit (alongside sweeping, painting, fixing, trading).
  - add: Add CraftingRecipe data {inputs:[liquid, blade|tissue] -> output: syringe|medikit}; a CraftSystem consumes inputs and spawns the consumable; tag outputs with the health/spirit deltas above.
  - src: HT3 Versions patch notes / wiki (combine liquids with blades/tissue -> syringes/medikits); editor-unlock article (credit topped by tasks/sweeping/painting/fixing/crafting/trading)

- **Fire extinguisher (and similar props) as THROWN EXPLOSIVES / environmental hazards** _(low)_ — The fire extinguisher can't put out fires but EXPLODES when thrown or fallen on — an environmental explosive prop. Combined with the broader 'props-as-weapons' backlog, specific objects have unique hazard behaviours (explode-on-impact) rather than being generic blunt weapons.
  - add: Tag certain props with an Explosive component {triggerOnThrow, triggerOnHeavyImpact, radius, damage}; PropImpactSystem detects throw/fall impact and emits an AoE damage+knockback event.
  - src: hard-timegame.fandom.com/wiki/Fire_Extinguisher (explodes when thrown or fallen on)

- **Full character EDITOR unlocked after completing one sentence — edit EVERY character in the city** _(medium)_ — Beating a sentence legitimately unlocks an in-built Editor that SAVES changes to every character in the world (persistent), not just yourself. Editable fields: name, age (16-99), gender, height (4-8ft), role (Inmate/Civilian/Guard), territory (N/S/E/W/None), gang, reputation, STR/INT/AGI, Stance (idle pose), Walk style, costume (Prison/Civilian/Police), and Moves (attacks, crushes, main/special/ground). You can undress any character to underwear.
  - add: Build an EditorMode that reads/writes the full component set on any selected entity and persists to the save; gate it behind a 'sentenceCompleted' flag on the profile.
  - src: mdickie.fandom.com/wiki/Editor; HT3 manual (editor unlocks after first sentence); editor-unlock guide (save file AppData/LocalLow/MDickie/Hard Time III)

- **Secret areas / environmental puzzles via prop-stacking and vents** _(low)_ — Players reach hidden/restricted areas by stacking props (e.g. build a chair tower to get above a safe room), blasting out a vent fan and slide-jumping in or stacking a barrel under it, and flipping hidden switches to open doors. Emergent traversal using the physics/prop system, not scripted doors.
  - add: Make props physically stackable (climbable colliders), add Vent entities with a destructible cover + traversal trigger, and HiddenSwitch entities wired to door open events; let pathing/jump use stacked geometry.
  - src: Steam Community 'No-video Secrets Guide' (chair tower above safe room, vent fan blast + slide-jump, hidden switch opens bathroom door)

- **Romance / advanced dialogue relationships (kiss, hug, marry) as a social-progression layer** _(medium)_ — HT3 has an advanced dialogue system where you develop relationships; romantic relationships between consenting adults involve kissing and hugging (and forming bonds/gangs). This is a positive social track that feeds spirit and ally formation, beyond the combat-only ally-on-sight model.
  - add: Add a Relationship component (per-pair affinity); DialogueSystem branches (greet/flirt/threaten) shift affinity; high affinity unlocks Romance status (spirit regen + ally-without-gang flag).
  - src: HT3 manual / Steam guide (romantic relationships, kissing/hugging, maintain bonds, form gangs via dialogue)

- **Overeating causes vomiting and STAT LOSS (need overshoot penalty)** _(low)_ — Eating is needed but must be MODERATE — overeating makes the character vomit and lose stats. So the hunger need has an upper-bound penalty, not just a lower-bound (starvation) one; bingeing on cafeteria/commissary food backfires.
  - add: In the Hunger need, add an upper threshold; FeedSystem triggers a Vomit event (temporary stat debuff + hygiene hit) when satiety exceeds it.
  - src: touchtapplay.com Hard Time tips (eat moderately; overeating causes vomiting and stat loss)

- **Costume/role state changes physical access and legal standing (post-release civilian + warden recruitment)** _(medium)_ — After serving your sentence you switch to civilian clothes with limited world access (only your house, located by the furniture store), and a guard may then offer to recruit you as a WARDEN. As a warden you're still arrestable (assault/murder charges apply) — role is a mutable state with its own access rules and liabilities, forming a meta-loop beyond a single sentence.
  - add: Add a Role component (Inmate|Civilian|Guard) that gates ZoneAccess and arrest rules; a LifecycleSystem transitions role on sentence end and offers warden recruitment, re-using the same charge/arrest pipeline against guards.
  - src: mdickiegameguide.blogspot.com (post-release civilian, house by furniture store, warden recruitment, wardens not untouchable)

- **Gore-tier instant-kill decapitation + lockdown mass-death 'death-save' quirk** _(low)_ — With gore setting high enough, an inmate can be INSTANTLY KILLED via decapitation during a fight (a low-probability one-hit-kill gated by a setting). Separately, in 3D versions there's an emergent bug-feature where when lockdown ends, everyone in an area except the player can suddenly drop, and the game's 'death saves' decide if they get back up — emergent mass-casualty events tied to the lockdown transition.
  - add: Add a gore-gated lowChance instant-kill roll on heavy hits (config flag); on lockdown-end, run a DeathSaveSystem that rolls survival for distressed NPCs.
  - src: TV Tropes VideoGame/HardTime (gore decapitation instant kill; lockdown-end mass drop + death saves)


## Detailed combat mechanics missed: wrestling move-set system, HT3 guns/taser/fire, dismemberment, riot shields, throwing, environmental kills

- **Directional move-set system (one button + direction = different move)** _(high)_ — Combat is not a fixed combo system. ATTACK, RUN, and PICK-UP each execute a DIFFERENT move from the character's personal move-set depending on whether a direction is held and which one. ATTACK alone aims low, ATTACK+direction aims high. Every character has a customizable per-slot move-set (front grapple, rear grapple, running, momentum, flying, flying-counter, front-drag, rear-drag, ground), so identical inputs produce different signature moves per inmate. This is the core of why Hard Time combat feels deep with only ~4 buttons.
  - add: Give each Combatant a MoveSet component keyed by (button, position, hasDirection, isRunning, isAirborne); combat system resolves the move by looking up the slot rather than a fixed attack.
  - src: HT3 v1.1.5 manual (mdickie.com/guides/hardtime3.pdf) Controls + Grappling; WR3D manual move slots; supercheats WR3D moves page

- **Grapple transitions, position changes (FOCUS), and struggle-direction tug-of-war** _(high)_ — Holding ATTACK on grapple impact transitions into ANOTHER hold; further transitions depend on each party's Skill, size difference, and the DIRECTION each is struggling in (a continuous tug-of-war, not a fixed animation). The FOCUS command inside a grapple flips your position front<->back. Releasing GRAPPLE with a direction whips/sends them running that way instead of just letting go. Some holds let BOTH people keep throwing strikes with ATTACK while clinched.
  - add: Model a grapple as a state with (attacker, victim, position, struggleVectorA, struggleVectorB); each tick re-evaluate transition table weighted by Skill+size+struggle direction; FOCUS toggles position flag.
  - src: HT3 v1.1.5 manual Grappling section; WR3D manual Grappling/Final Destinations

- **Parry/counter by holding GRAPPLE into an incoming attack; ground-rise reversal** _(high)_ — Holding GRAPPLE (defensive) while an attack lands raises the chance it is parried or countered, scaled by Skill/size. Retreating from an opponent independently raises block chance. Holding GRAPPLE while on the ground raises the chance of instantly springing up INTO a move (reversal from prone). Wardens are NOT immune to parries (v1.1.5 fix).
  - add: Add a defensiveStance flag set while GRAPPLE held; on incoming hit roll counterChance = f(skill, sizeDiff, retreating, grounded); on success swap to counter-move.
  - src: HT3 v1.1.5 manual Grappling + Combinations ('Retreat to increase blocking'); WR3D Reversals

- **ATTACK+RUN power attack and 'lash out behind' modifier** _(medium)_ — Pressing ATTACK+RUN together launches a single powerful (charged) attack distinct from normal strikes. On a controller, holding FOCUS while attacking (unarmed) deliberately lashes out BEHIND you — letting you hit someone grabbing you from the rear without turning.
  - add: Detect simultaneous ATTACK+RUN -> spawn heavyAttack variant; FOCUS+ATTACK -> target rear-arc instead of facing arc.
  - src: HT3 v1.1.5 manual Combinations + Version History (FOCUS+attack lash-out-behind)

- **Gun system: hold-to-aim/release-to-fire, naturalistic auto-aim, move-while-shooting accuracy penalty** _(high)_ — Shooting uses hold ATTACK to aim, release to fire one round with 'relative accuracy'. NO crosshair — a 'naturalistic' system where simply FOCUSing an opponent and firing auto-hits. If you held a movement direction BEFORE firing you keep moving but with reduced accuracy; standing still first lets you fine-tune aim with directions. You can fire while sitting/kneeling. Aim up/down can be inverted in options. Guns have limited ammo and v1.1.5 reduced ammo + made reloads more frequent to stop spamming.
  - add: Add Ranged component (ammo, aimCharge, accuracy); on FOCUS target, hit roll = baseAcc * (moving?0.6:1) * skill; release fires projectile with limited ammo.
  - src: HT3 v1.1.5 manual Shooting section

- **Continuous-fire weapons (machine gun, flamethrower) vs single-shot** _(medium)_ — Machine gun and flamethrower fire CONTINUOUSLY while ATTACK is held instead of the hold-and-release single-shot model. Flamethrower has short range but sets enemies on fire and deals heavy/sustained damage (can dismember at long range in the sibling Extra Lives engine).
  - add: Flag weapons as fireMode: single|continuous; continuous emits a damage/fire stream each tick while held instead of one projectile on release.
  - src: HT3 v1.1.5 manual Shooting; Weapons fandom / Extra Lives fandom

- **Taser and batteries-as-universal-electric-ammo** _(medium)_ — An electric taser stuns opponents. Batteries reload tasers — but batteries can also be inserted into ANY other gun to make it fire electricity instead of bullets. Ammo type is swappable via the crafting/combine system, so a pistol loaded with a battery becomes a stun weapon.
  - add: Make ammo a typed resource (bullet/electric/etc); weapon.fire reads loaded ammoType -> applies stun vs damage effect; reload = combine(weapon, ammoItem).
  - src: HT3 search result from manual/fandom; HT3 manual crafting/reload

- **Riot shield blocks ~95% of attacks** _(medium)_ — The riot shield is a defensive prop that blocks roughly 95% of incoming attacks while held — a near-hard counter to melee that guards (and players) use, making shielded enemies require flanking, grappling, or ranged/fire to defeat.
  - add: Shield prop adds blockChance ~0.95 to front arc only; attacks from rear/flank or grapples bypass it.
  - src: Hard Time fandom Weapons / search result ('Riot Shield blocks 95% of attacks')

- **Sharp-vs-blunt weapon distinction drives bleeding; specific melee tiers** _(high)_ — Sharp weapons (knife, sword, shovel) deal far more damage AND cause the target to BLEED; knife and sword are the most lethal melee. Blunt weapons (chair, pipe, bat) deal blunt/stun damage without the bleed. Firearm bullets all deal similar per-hit damage but differ in fire RATE (machine gun > shotgun > pistol). Using melee on guards risks being grabbed and arrested at close range.
  - add: Tag weapons with damageType (sharp/blunt/ballistic) and fireRate; sharp adds a Bleeding component to victim; blunt adds Stun; guards within grapple range of an armed attacker trigger arrest attempt.
  - src: Hard Time fandom Weapons; MDickie game guide blogspot

- **Dismemberment / missing limbs as a persistent state that disables abilities** _(high)_ — Strong enough attacks, explosives, and the flamethrower can sever limbs. Missing limbs are a persistent body-state: v1.1.5 explicitly notes 'missing limbs cannot be used in crush attacks' — i.e. losing an arm removes specific move capabilities. Gore correctly originates from the impacting body part (fists wrapped in chains/metal produce gore).
  - add: Add BodyParts component with per-limb intact flags; sever on overkill/explosive/fire; move resolver skips moves whose required limb is missing.
  - src: HT3 v1.1.5 Version History; Weapons/Extra Lives fandom

- **Crush attacks on downed/grounded opponents (stomps) and pinning predicaments** _(medium)_ — Distinct 'crush' attacks exist for hitting opponents who are on the ground (stomps/limb-targeted), separate from standing strikes — and these are what require an intact limb. Pinning/predicament states resolve into a hold where possible (v1.1.5). This is a separate ground-combat phase from standing.
  - add: When target is Grounded, ATTACK resolves to crush/stomp move targeting a body part; gate on attacker limb intact.
  - src: HT3 v1.1.5 Version History ('crush attacks', 'pinning predicaments end in a hold')

- **Pick-up / throw / overhead-lift / combine-objects (crafting reload) as a unified prop verb** _(high)_ — PICK-UP with a direction THROWS the held object; holding PICK-UP lifts furniture/people OVERHEAD if STR is sufficient; holding RUN while carrying furniture stops you turning with it (positioning for a throw). Holding RUN+PICK-UP COMBINES objects — this is the same input that reloads guns and crafts. Objects that smash in-hand are removed; objects auto-swap hands to favor the most useful one (e.g. food to mouth). Thrown/dropped/burned explosives detonate.
  - add: Carried prop is an entity in a Hands component; PICK-UP+dir applies throw impulse+collision damage; RUN+PICK-UP invokes combine(itemA,itemB)->recipe (reload/craft/explosive).
  - src: HT3 v1.1.5 manual Combinations + Version History

- **Fire as a deliberate weapon/hazard: TAUNT+PICK-UP ignites objects; fire spreads** _(high)_ — Holding TAUNT+PICK-UP deliberately sets fire to a small held object; fire can be set to flammable objects including large furniture, and spreads/burns NPCs to death. Explosives cause explosions that dismember or ignite nearby NPCs. Fire is an environmental hazard, not just a flamethrower effect.
  - add: Add Flammable + OnFire components; OnFire deals DoT, can sever limbs at high stack, and propagates to adjacent Flammable entities each tick; explosives apply radial fire+dismember.
  - src: HT3 v1.1.5 manual Combinations ('set fire to a small object'); fandom search result on fire/explosives

- **Adrenaline rush (special-move trigger) and nervous breakdown (lose control)** _(high)_ — The thin amber/yellow meter under health is MENTAL health. Filling it via positive experiences triggers an 'adrenaline rush': +10% to every stat temporarily AND every move you trigger becomes a SPECIAL move or a stronger variant of the normal move. Conversely, if spirit is exhausted the character has a nervous breakdown and you literally LOSE CONTROL of them (AI takes over). This is the Hard Time equivalent of the wrestling special/finisher meter.
  - add: Add Morale meter; threshold high -> apply +10% stat buff + flag moves as special variant; threshold zero -> detach player input and run AI for a duration.
  - src: HT3 v1.1.5 manual Adrenaline section

- **Grapple-to-arrest / interrogate (TAUNT-in-hold) and intimidation grabs** _(medium)_ — Inside a grapple, TAUNT applies a hold and continuing to hold it ARRESTS or INTERROGATES the victim — this is literally how police make arrests (grab + hold command). Intimidating characters are more likely to get their way via a hold at the cost of the relationship. RIGHT-STICK/SPACE in a grapple = arrest. So grappling is dual-use: combat AND a social/coercion verb.
  - add: Add a holdHeldTimer to grapple; on TAUNT held past threshold trigger Arrest/Interrogate outcome weighted by Reputation/intimidation.
  - src: HT3 v1.1.5 manual Grappling ('TAUNT to apply a hold then arrest/interrogate') + Conversations

- **Wrestling-lineage finisher/submission specifics (special meter, desperate finisher, tap-out, rope crawl)** _(medium)_ — Special meter fills from landing attacks AND from taunts; spend it via any front/rear grapple to fire a finisher, OR press the desperate-finisher input (space/TAUNT) for a low-success early attempt. Submissions: tap-out is probabilistic from victim attributes — apply pressure with directions; mash ATTACK/RUN/PICK-UP (NOT grapple, which releases) to deepen the hold; the victim mashes to escape and can crawl to the ropes/edge to break. Named example moves in the engine: brainbuster (front drag), dragon/reverse suplex (rear drag), tombstone piledriver / Death Valley Driver (finisher), spear/dropkick (running/flying), codebreaker (flying counter), head stomp (ground special).
  - add: Add SubmissionHold with pressure value increased by attacker mashing and victim distanceToBreakPoint; tapChance = f(victimAttr,pressure); finisher gated by special meter spent on grapple move.
  - src: WR3D manual (engine-shared) + supercheats/Steam discussion move guides


## Emergent NPC Life + Meta (Hard Time series): status spread, NPC routines/jobs, death/inheritance/persistence, gang wars, snitching, romance/jealousy, riots/escapes, difficulty/sentence/charging

- **Two-axis social standing: Reputation (feared/respected) vs Attitude (good-citizen)** _(high)_ — Every character has BOTH a Reputation stat ('how respected or feared you are... influences how likely you are to get your own way') AND a separate Attitude stat ('a more positive version of your reputation for doing the right thing'). Winning fights/accomplishing tasks raises Reputation; cooperating/not letting people down raises Attitude. Crucially these are opposed by audience: high Reputation makes convicts fear you but makes the judge MORE likely to increase your sentence. This is distinct from a single 'reputation/notoriety' value — it's two ledgers that the same action moves in opposite directions for different NPC factions.
  - add: Add ReputationComponent{respect, attitude} and have an OpinionSystem compute each observer's reaction as f(observerFaction, actorRespect, actorAttitude) so guards/judge weight attitude-negatively while inmates weight respect-positively.
  - src: HT3 manual (mdickie.com/guides/hardtime3.pdf) 'Attributes' section; mdickie.fandom.com/wiki/Hard_Time

- **Reputation is a spendable currency in dialogue; bothering people drains it** _(medium)_ — Manual: 'being rejected will damage your reputation — so it's like a currency that you spend to get your own way' and 'if you bother people too much it will damage your attitude.' Initiating conversations, making requests, or being refused costs standing. Intimidation (grab + hold command) lets intimidating characters get their way 'at the expense of the relationship.'
  - add: Each social request rolls success vs target's fear/respect-of-actor; on failure decrement actor.respect and on pestering decrement actor.attitude, so spamming dialogue is self-punishing.
  - src: HT3 manual 'Conversations' section

- **Guilty-verdict pride: innocent verdicts LOWER inmate respect** _(medium)_ — Fellow convicts only respect a guilty man and 'will look down on innocent verdicts (or even be suspicious of them).' So beating the rap in court is socially costly inside — the inmate world rewards being convicted. Sentence baseline ~50 days at Southtown ('short doses of hard time, months not years').
  - add: On verdict event, if acquitted apply a respect penalty + suspicion flag with inmate NPCs; if convicted apply small respect bonus scaled by charge severity.
  - src: mdickie.fandom.com/wiki/Hard_Time; search snippet on guilty/innocent respect

- **Crimes accumulate as a buildable case even if unwitnessed; escalating warrant tiers** _(high)_ — 'Even if you think you got away with something, it may only be a matter of time before the police build a case against you.' Consequences escalate: warning -> beating -> full arrest warrant where 'sirens blare and every officer is alerted to hunt you down.' City-wide announcements (arrest warrants) are audible anywhere. Witnessing can be by an officer OR 'an especially vindictive peer' (this is the snitch vector).
  - add: Give each crime a SuspicionLevel that ticks up over time even without a live witness; cross thresholds to escalate WardenAlertState (warn->beat->warrant=global aggro broadcast).
  - src: HT3 manual 'Crime & Punishment' section; v1.0.5 'Important announcements... can be heard anywhere'

- **Snitching = a 'vindictive peer' acting as witness against you** _(high)_ — Crimes can be reported not just by officers but by hostile inmates ('an especially vindictive peer'). So a damaged relationship turns an NPC into an informant who feeds the case-building system. This is the concrete snitching consequence: enemies you've made will testify/report, converting social grudges into legal jeopardy.
  - add: When a crime is observed, any witness with relationship<threshold rolls to file a report adding to your SuspicionLevel; reward the snitch with guard-attitude or cash.
  - src: HT3 manual 'Crime & Punishment'; touchtapplay guide (inmates report contraband)

- **Court drama: judge may reject charges; jury reacts to prosecution/defence not just verdict** _(medium)_ — 'your case is presented to a judge who may or not agree with the charges.' Punishments range 'from physical limitations to fines and sentencing.' v1.1.3: 'The jury reacts to the prosecution & defence as well as the verdict.' Intelligence/Skill historically improves court-defence success. Conviction can be avoided entirely.
  - add: Model a TrialSystem where each charge has an independent conviction roll modified by defendant Skill and evidence/witness count; outcome applies fine, stat-debuff, or sentence delta.
  - src: HT3 manual 'Crime & Punishment'; v1.1.3 changelog

- **Death feeds a persistent 'Hall of Fame'; dying inmates may stay dead across inherited universes** _(high)_ — On death the character 'fades more gradually into the Hall Of Fame with melancholy music' and joins it as a deceased entry others can find; undiscovered characters stay hidden there. Universe persistence: 'Inherit Universe' regenerates the last universe with every character and weapon in the same position; deaths may or may not persist (death-saves). Achievements/unlocked characters are stored SEPARATELY from the universe so restoring defaults doesn't lose them.
  - add: Split persistence into WorldState (mutable, inheritable, holds living/dead NPCs in-place) and a global HallOfFame/unlock ledger that survives world resets; on death move entity to a Deceased archetype recorded in both.
  - src: mdickie.fandom.com/wiki/Hard_Time (universe options); v1.1.5 'Undiscovered characters remain hidden in the Hall Of Fame'; HT3 manual 'Universes'

- **Character inheritance: take over a (often dead) character keeping their developed stats** _(medium)_ — 'Inheriting an existing character leaves their stats intact' — you can assume a built-up character rather than starting fresh, so a dead inmate's trained STR/SKILL/etc. can be inherited by the next playthrough's protagonist. Death-survival can transform you: a cop who dies respawns as a bulletproof RoboCop cyborg (unlocks robotic editor parts; cyborgs feel cold 50% less but aren't bulletproof vs water/liquids).
  - add: Add an inherit-character path at new-game that deep-copies a chosen entity's attribute set; on player death roll a survival/transform outcome (hospital-injured, dead, or transformed archetype).
  - src: v1.1.2 'Inheriting an existing character leaves their stats intact'; v1.0.7 cyborg cop; v1.1.4 cyborg cold

- **Surviving death in hospital halves your bank balance** _(low)_ — v1.1.2: 'Surviving death in hospital halves your bank balance.' Concrete economic cost for the near-death save path, distinct from injuries. Hospital also offers surgery (heal injuries) and therapy (boost attributes) as paid services.
  - add: On near-death-revive event, set money *= 0.5 and route entity through Hospital with optional paid surgery/therapy interactions.
  - src: v1.1.2 changelog; HT3 manual 'Health'

- **Gang leadership hierarchy with boss-only privileges, succession, alliances and declared WARS** _(high)_ — Gangs can 'form alliances or declare war on each other.' Only the leader may set the gang name, greeting (unique handshake gesture), and dress code; leaders can't be assigned missions by members and have an increased chance of success when demanding money from members. Vacant leaders are auto-replaced by another member; your active reign is unlikely to be replaced without warning. Storyline: members conspire about who the leader should be. Defeated opponents can be offered a chance to survive by JOINING your gang (forced recruitment). Joining a gang changes your costume in real-time.
  - add: Add GangComponent{id, leaderId, allies[], enemies[]} + a GangPoliticsSystem handling succession on leader-death, alliance/war state affecting allies-on-sight, and a 'submit-or-join' offer on KO.
  - src: v1.1.0/v1.1.5 changelog (gang war/alliance, leader privileges, succession); HT3 manual 'Relationships'

- **Betrayal is execution-worthy; questioning affiliation; war makes whole gangs hostile on sight** _(medium)_ — Betraying a gang or joining a rival 'risks execution' and 'triggers persistent violence.' Gang members can be questioned about their affiliation (intel-gathering). Allies only rush to aid 'if they can see clearly enough' — line-of-sight gates rescue. Knowing if a target is 'well-connected before you start a fight' is strategically vital because their whole network may swarm you.
  - add: Tag betrayal events to set a persistent Vendetta flag across the betrayed gang; gate ally-assist on a vision/LoS check; expose an 'ask affiliation' dialogue that reveals GangComponent.
  - src: mobile guide (betrayal revenge); HT3 manual 'Relationships'; v1.1.0 'gang members can be questioned'

- **Romance emerges from repeated pleasing; jealousy if a third party witnesses affection** _(medium)_ — 'Every time you please someone, the relationship also has a chance to become romantic — with hugs & kisses that bring health & happiness... so long as you're not witnessed by somebody else who might be jealous.' So affection in view of a jealous party causes conflict. 'Partner' relationship status follows you scene-to-scene; you can proactively propose partnership. Pregnancies can be hinted at before they happen; offspring cost up to $100 paid only to mothers you're on good terms with; siblings/parents are tracked ('Mum/Dad', 'siblings').
  - add: Add a Romance state on the relationship edge; affection interactions emit a public event that any onlooker with a romantic edge to either party reacts to (jealousy -> hostility); model Partner/parent/sibling family edges that persist across scenes.
  - src: HT3 manual 'Relationships'; v1.1.2 Partner; v1.0.7 offspring $100; v1.1.4 siblings; v1.1.5 pregnancy hints

- **Adrenaline/spirit two-state: rush (+10% all stats, special moves) vs nervous breakdown (lose control)** _(medium)_ — The thin yellow meter is mental health. Filled by positive experiences -> 'adrenaline rush': temporarily 10% stronger in every respect and moves upgrade to special/stronger versions. Emptied -> 'nervous breakdown' where you literally 'lose control' of the character (AI takes over). This applies to NPCs too and drives emergent breakdowns.
  - add: Add Spirit meter to all actors; at high threshold apply a timed +10% buff + move-upgrade flag, at zero hand control to AI with erratic behavior for both player and NPCs.
  - src: HT3 manual 'Adrenaline'; v1.1.2 breakdown lose-control

- **Daily fashion meta: an item becomes fashionable/unfashionable each day; dressing wrong costs reputation** _(low)_ — 'Every day, something becomes fashionable or unfashionable so you may want to dress accordingly' to preserve reputation. Also: prison outfits must be painted your block's color; deviating draws warden criticism, and wearing a vest/impersonating an officer is itself a chargeable offense. Even certain MOVES can be 'fashionable' (v1.0.5 note: non-existent moves can't be fashionable).
  - add: Add a daily-rotating FashionTrend{itemId or moveId}; OpinionSystem grants small respect bonus/penalty based on whether the actor's worn item matches today's trend; flag off-color/impersonation outfits as crimes.
  - src: HT3 manual 'Fashion'; v1.0.4 vest=impersonation

- **NPC routines mirror the player's full schedule; CPUs sleep at night regardless of energy** _(medium)_ — Seamless real-time day ~8am-8pm; you wake in your own cell and must return to it. Wardens can reassign your cell/block AND adjust the schedule on the fly (lockdown time itself can change over time). NPCs follow the same loop: 'CPU characters stay asleep during the night regardless of their energy levels' and inmates must be in their assigned cell overnight (return-to-correct-level logic exists). Gang members 'accompany you more often' (they tag along during the day).
  - add: Give NPCs the same ScheduleComponent the player uses (wake/work/eat/return-to-cell/sleep) with a forced night-sleep override; let WardenSystem mutate an NPC's assignedCell/block/scheduleWindow.
  - src: HT3 manual 'Schedule'; v1.0.5 CPU sleep; v1.0.4 lockdown schedule can change; v1.1.5 gang members accompany

- **Jobs/income are virtues that can be invalidated; cop is the top job and is paid per conviction** _(medium)_ — Income paths: tasks/missions, sweeping floors, painting art, fixing furniture, crafting, trading possessions. But sweeping floors steadily DAMAGES reputation (menial = low status), and 'fixing your own bed/toilet is not a virtue you can be paid for' (can't farm self-repairs). Police work is 'the most lucrative role,' paid 'for successful convictions and helping to solve other cases' — and you get recruited to be a warden/cop by repeatedly visiting. Wardens themselves can be charged with crimes (assault/attempted murder).
  - add: Model jobs as virtue-actions paying cash but applying a per-job respect delta (sweeping negative); gate self-repair from payout; add a Cop role whose income = bounty per resolved case and who is still subject to the crime system.
  - src: HT3 manual 'Finances'; mobile guide (sweeping lowers rep, warden recruitment, wardens chargeable)

- **Emergent major incidents: bomb threats, floods/fire, and a Purge-style 'Immune System' legal-amnesty day** _(medium)_ — v1.0.8: 'New major incidents include bomb threats!' and 'In rare situations, crime can become legal for one day under a Purge style Immune System.' Floods are timed world events (v1.1.4 'adapted the timing of floods'). Fire is a full hazard system: deliberately set fire to any flammable object/large furniture (PICK-UP+ATTACK), fire damages health but heat also gives a sleeping health bonus and offsets cold; fire extinguishers/flamethrowers exist. Cold weather outdoors drains health (discolored meter + shivering); underwater = 25% colder.
  - add: Add a WorldEventScheduler emitting rare global events (BombThreat, Flood, PurgeDay=disable-crime-system-for-1-day); add Temperature + Fire-spread components feeding the health/sleep systems.
  - src: v1.0.8 incidents/Purge; v1.1.2 cold/fire; v1.1.4 floods/underwater cold

- **Break-INS as well as break-outs: intruders tunnel up through the ground into the prison** _(medium)_ — v1.1.5: 'Intruders are more likely to tunnel up through the ground — especially when breaking into or out of prison.' Escape/intrusion is symmetric and emergent: NPCs tunnel both directions. Tunnel escape (find shovel in workshop, dig FLOOR not walls at block extremities) is lower-risk than the midnight fence climb (high capture/injury risk). Fewer cops now swarm to avenge a colleague's death (crowd-control on guard response).
  - add: Make escape an emergent NPC behavior too: any inmate with high enough motivation can attempt fence/tunnel; spawn outside intruders that tunnel in; cap simultaneous guard responders to a death.
  - src: v1.1.5 intruders tunnel; mobile guide (fence vs tunnel escape); v1.0.9/v1.0.6 cop swarm tuning

- **Mystery/whodunit subsystem driven by the phone and 'Guess Who?' clues** _(low)_ — v1.0.8: answering the phone can trigger a mystery where 'Guess Who?' style clues help you figure out who to confront; you solve it by grabbing the suspect and taunting (like an arrest). v1.1.5: 'Mysteries are only solved when the STAR character grabs the suspect (not a CPU).' This is an emergent investigation loop layered on the phone/visiting system.
  - add: Add a MysterySystem that on phone-answer picks a hidden culprit + emits deduction clues; resolution = player-initiated grab+taunt on the correct entity, rewarding rep/cash.
  - src: v1.0.8 mysteries via phone; v1.1.5 star-only solve

- **Create-a-character / in-world editor gated behind completing a sentence legitimately; 3 outfits per character** _(low)_ — Beating a sentence the proper way UNLOCKS an Editor to rewrite every character in the city (and a Fight-Scene sandbox mode). Editing also happens live via wardrobes/mirrors/hairdryers/lockers during play, and at game start in limited form. Each character has 3 separate costume slots; you can clone/copy outfits across characters (even across genders); gang join recolors in real-time. PC can scale population to 200% (more NPCs).
  - add: Persist an unlock flag on legitimate-release that enables a runtime entity editor; give CostumeComponent 3 slots + clone op; expose live edit at wardrobe/mirror props.
  - src: HT3 manual 'Editing' & 'Fight Scenes'; v1.1.3 200% population

- **The 25%-tired penalty is universal AND attributes are health-capped (amber ceiling on recovery)** _(medium)_ — Confirms/extends your 25% rule: attributes 'are only as good as their health and could be up to 25% weaker when tired' — this caps ALL attributes (incl. Reputation/Attitude effectiveness), not just combat stats. Health has an amber ceiling: 'the longer your health has been low, the harder it is to restore fully (shown by an amber limit)'; only good sleep on a comfortable bed restores it fully; in v1.1.0 the amber drain became optional/off-by-default making you responsible for tiring yourself out to sleep.
  - add: Compute effective stat = base * (1 - 0.25*tiredFrac) for every attribute including social; track a sticky amber max-health ceiling that only full sleep raises.
  - src: HT3 manual 'Attributes' & 'Health'; v1.1.0 amber optional


## Ranked
1. **Two-axis social standing: Reputation (feared/respected) vs Attitude (good-citizen), opposed by audience** — AUTHENTICITY: maximal. This is the keystone the whole social/legal sim hangs on. The defining Hard Time feel is that violence and a guilty verdict make you respected by inmates but feared/punished by guards and the judge, while being a model citizen makes you safe legally but weak inside. EFFORT: medium. You already have a reputation value, so this is splitting it into {respect, attitude} and adding an OpinionSystem that weights them per observer faction. Verified: the warden/court behavior I confirmed only makes sense with this split. Unlocks ranks 2,3,5,7 below.
2. **Crime builds a case over time even when unwitnessed; escalating warrant tiers (warning to beating to global arrest warrant) with a city-wide broadcast** — AUTHENTICITY: very high. 'Even if you got away with it, the police may build a case' plus the siren/global-alert warrant is iconic and turns crime into a slow-burn risk rather than a binary caught/not-caught. You have guard detection but not delayed suspicion accrual or warrant escalation. EFFORT: medium. A SuspicionLevel per crime that ticks up over time and crosses thresholds (warn -> beat -> global-aggro broadcast). Pairs directly with snitching (rank 5).
3. **Spirit / mental-health second bar: fill it for Adrenaline Rush (+10% all stats + every move upgrades to a special variant); empty it for Nervous Breakdown (you LOSE CONTROL, AI takes over)** — AUTHENTICITY: very high. This is the wrestling-lineage finisher meter and the source of the most memorable emergent moments (yours and NPCs' breakdowns). Your backlogged 'morale two-bar' is the placeholder for this, but the two named end-states (rush special-moves, breakdown control-loss) are what make it Hard Time, not a generic morale bar. EFFORT: medium. A Spirit meter on all actors with a high-threshold buff/move-upgrade flag and a zero-threshold controller swap to AI for N ticks. Applies to NPCs too, which multiplies the payoff.
4. **Warden release CONDITIONS as a timed petition/parole system (raise stat to X / drop rep below X / accumulate $X by day Y; meeting shaves days, failing ADDS days with no court case; plus bribery and 'copy my hairstyle or I pin a crime on you')** — AUTHENTICITY: high and INDEPENDENTLY VERIFIED against the wiki, which confirmed even the obscure extras (bribes, hairstyle-copying, threats to frame you). This is the real parole loop and is distinct from your generic jobs and your backlogged court/parole. EFFORT: low-medium. A ReleaseConditions list on the player + a WardenContractSystem that rolls conditions and applies +/- days to Sentence on deadline. High ratio of authenticity to code.
5. **Snitching: a 'vindictive peer' (any inmate with a damaged relationship) acts as a witness and files a report that feeds the case-building system** — AUTHENTICITY: high. This is the concrete payoff that converts social grudges into legal jeopardy and is what makes the relationship system have teeth. You have 'allies-on-sight' but no enemy-informant vector. EFFORT: low once ranks 1+2 exist. On an observed crime, any witness with relationship below a threshold rolls to add to your SuspicionLevel, optionally rewarded with guard-attitude or cash. Nearly free given the suspicion system.
6. **Directional move-set system: ATTACK / RUN / PICK-UP each fire a DIFFERENT move per character depending on direction held, running, and airborne state (per-slot customizable move-set: front/rear grapple, running, momentum, flying, ground)** — AUTHENTICITY: very high — this is literally why ~4-button combat feels deep and why every inmate has signature moves. EFFORT: high. It restructures combat from fixed attacks into a MoveSet component keyed by (button, position, hasDirection, isRunning, isAirborne) plus a grapple transition table weighted by Skill/size/struggle-direction. Ranked below cheaper wins because of cost, but it is the single biggest COMBAT authenticity lever; do it when you touch the combat core.
7. **Hall of Fame on death + persistent/inheritable universe + character inheritance (take over a built-up, often dead, character keeping their stats)** — AUTHENTICITY: high and it is the meta-loop that makes the world feel persistent and consequential. Your 'persistent-death world' backlog is the seed, but the specifics (deceased entries become discoverable Hall-of-Fame characters; Inherit Universe regenerates everyone/everything in place; new protagonist can inherit a dead inmate's trained stats; unlocks stored SEPARATELY from world state so resets don't wipe them) are what give it Hard Time identity. EFFORT: medium-high — requires splitting persistence into mutable WorldState vs a global HallOfFame/unlock ledger.
8. **Grapple as a dual-use COMBAT and COERCION verb: hold GRAPPLE to parry/counter (scaled by Skill/size), FOCUS flips front/back position, and TAUNT-in-hold ARRESTS or INTERROGATES the victim (this is literally how police arrest)** — AUTHENTICITY: high. Grappling being how you BOTH fight and arrest/intimidate/interrogate is core to the cop and gang fantasy. EFFORT: medium, and it rides on rank 6's grapple state; a holdHeldTimer plus a TAUNT-past-threshold outcome weighted by Reputation. Defensive-hold parries also fix the 'combat is just trading hits' problem cheaply.
9. **Jobs are virtues with social side-effects: sweeping floors pays but DRAINS reputation (menial = low status), self-repairs can't be farmed for pay, and Cop is the top job paid per conviction** — AUTHENTICITY: medium-high. The fact that the easy income (sweeping) costs you respect is a beautifully Hard Time tension your flat jobs list misses. EFFORT: low once rank 1 exists — attach a per-job respect delta (sweeping negative) and gate self-repair from payout. Cheap flavor that makes the economy feel moral.
10. **Fire and explosives as a deliberate, spreading environmental weapon (TAUNT+PICK-UP ignites a held object; fire spreads to flammable furniture and burns NPCs to death; fire extinguisher EXPLODES when thrown; explosives dismember/ignite)** — AUTHENTICITY: high for emergent chaos and riot/escape spectacle, and it ties together your weapons, props-as-weapons backlog, and lethal-KO. EFFORT: high — needs Flammable/OnFire components with per-tick DoT and propagation, plus Explosive props and dismemberment body-state. Ranked last of the top set on effort, but it is the biggest single source of emergent 'anything can happen' moments and pairs with the dismemberment-disables-moves system (missing limbs can't perform crush attacks).
